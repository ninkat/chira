...
import moviesData from '@/assets/movies.json'; // updated data import
...
const Movies: React.FC = () => {
  ...
  useEffect(() => {
    ...
    tooltipGradient
      .append('stop')
      .attr('offset', '0%')
      .attr('stop-color', '#2d3748') // darker gradient for movie theme
      .attr('stop-opacity', 0.98);

    tooltipGradient
      .append('stop')
      .attr('offset', '100%')
      .attr('stop-color', '#1a202c') // very dark base
      .attr('stop-opacity', 0.98);
    ...
    tooltipContent
      ...
      .attr('fill', '#e2e8f0') // lighter text for dark bg
    ...
    textLineClasses.forEach((className, index) => {
      tooltipContent
        ...
        .attr('fill', '#a0aec0') // muted info text
    ...
    const updateSelectedNodesInfo = (nodes: D3Node[] | D3Node | null) => {
      ...
      if (nodesArray.length === 0) {
        ...
        // function to wrap text with proper line breaks (simplified)
    ...
    const updateVisualization = () => {
      ...
      nodeEnter
        ...
        .attr('fill-opacity', 0.85) // set opacity
        .attr('stroke', '#1A202C')
      ...
      linkMerge
        ...
        .attr('marker-end', (d) => {
          if (linksToHighlight.includes(d)) {
            return 'url(#movie-arrowhead)'; // all highlighted links get an arrow
          }
          // for non-highlighted links, only 'directed' type gets an arrow
          return d.type === 'directed' ? 'url(#movie-arrowhead)' : '';
        });
